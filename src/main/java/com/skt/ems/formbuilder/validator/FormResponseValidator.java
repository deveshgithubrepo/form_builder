...
public class FormResponseValidator implements Validator {
    @Override
    public void validate(DefaultRequest request) throws Exception {
        if (!(request instanceof CreateFormResponseRequest formResponseRequest)) {
            throw new ApplicationException(MessageKey.INVALID_FORM_REQ, HttpStatus.BAD_REQUEST);
        }
        if (StringUtils.isEmpty(formResponseRequest.getFormId())) {
            throw new ApplicationException(MessageKey.NULL_OR_EMPTY_FORM_ID, HttpStatus.BAD_REQUEST);
        }
        if (StringUtils.isEmpty(formResponseRequest.getUsername())) {
            throw new ApplicationException(MessageKey.NULL_OR_EMPTY_USERNAME, HttpStatus.BAD_REQUEST);
        }
    }
}
