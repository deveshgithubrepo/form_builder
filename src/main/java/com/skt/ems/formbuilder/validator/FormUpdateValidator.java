...
public class FormUpdateValidator implements Validator {

    @Override
    public void validate(DefaultRequest request) throws Exception {
        if (!(request instanceof UpdateFormRequest updateFormRequest)) {
            throw new ApplicationException(MessageKey.INVALID_FORM_REQ, HttpStatus.BAD_REQUEST);
        }

        if (StringUtils.isEmpty(updateFormRequest.getFormId())) {
